...
f
inline float get_tensor_max(int k, void* b_) {
...
inline void lut_ctor(int act_k, int8_t* qlut, bitnet_float_type* b, bitnet_float_type* lut_scales) {
    ...
    uint8_t tbl_mask[16] = {0, 2, 4, 6, 8, 10, 12, 14, 1, 3, 5, 7, 9, 11, 13, 15};
...
#pragma unroll
        for (int idx = 0; idx < 8; idx++) {
            ...
            vst1_s8(qlut + k * 256 + idx * 32, q0_high);
            vst1_s8(qlut + k * 256 + idx * 32 + 8, q1_high);
            vst1_s8(qlut + k * 256 + idx * 32 + 16, q0_low);
            vst1_s8(qlut + k * 256 + idx * 32 + 24, q1_low);
        }
    }
#endif
}
