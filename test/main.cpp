...
/* A(K/2 x M/2), B(1 x K)
   ...
   This version uses SIMD optimizations, parition the LUT construction among threads.
*/
void vecmul_lut_packed3(uint8_t* A, float32_t* B, float32_t* C, float32_t* ws, int M, int N, int K) {
    int KK = K / 2;
    const uint8x16_t vec_mask = vdupq_n_u8(0x0f);
    const float32_t weight_scale = ws[0];
    float32_t epsilon = 1e-7f;
...
*/
void vecmul_lut_packed4(uint8_t* A, float32_t* B, float32_t* C, float32_t* ws, int M, int N, int K) {
    int KK = K / 2;
...
/* A(K/2 x M), B(1 x K)
   QLUT(K*16), QLUT is contructed for each row of B. each K has 32 bytes (first 16 high bytes and then 16 low bytes)
        each K represents 2 activations in B. 
   C(1 x M)
   We unroll the k-loop by 8 to further increase the compute intensity and reduce loop overhead.
*/
void vecmul_lut_packed5(uint8_t* A, float32_t* B, float32_t* C, float32_t* ws, int M, int N, int K) {
    int KK = K / 2;
    const int stride = M / 2;
    const int lut_stride = 32;
    const uint8x16_t vec_mask = vdupq_n_u8(0x0f);
    ...
    #pragma omp parallel num_threads(4)
    {
        ...
        for (int i = 0; i < M; i += 128) {
            ...
            for (int k = 0; k < KK; k += 8) {
                // Prefetch A data and QLUT entries for the next k-iteration block
                if (k + 8 < KK) {
                    const uint8_t* pA_next = A + (k + 8) * stride + i_packed;
                    __builtin_prefetch(pA_next, 0, 3);
                    __builtin_prefetch(pA_next + stride, 0, 3);
                    __builtin_prefetch(pA_next + 2 * stride, 0, 3);
                    __builtin_prefetch(pA_next + 3 * stride, 0, 3);
                    __builtin_prefetch(pA_next + 4 * stride, 0, 3);
                    __builtin_prefetch(pA_next + 5 * stride, 0, 3);
                    __builtin_prefetch(pA_next + 6 * stride, 0, 3);
                    __builtin_prefetch(pA_next + 7 * stride, 0, 3);

                    // Prefetch QLUT entries (256 bytes)
                    __builtin_prefetch(QLUT + (k + 8) * lut_stride, 0, 3);
                    __builtin_prefetch(QLUT + (k + 8) * lut_stride + 64, 0, 3);
                    __builtin_prefetch(QLUT + (k + 8) * lut_stride + 128, 0, 3);
                    __builtin_prefetch(QLUT + (k + 8) * lut_stride + 192, 0, 3);
                }

                const int8_t* pQLUT = QLUT + k * lut_stride;
                int8x16x4_t ql0 = vld1q_s8_x4(pQLUT);
                int8x16x4_t qh0 = vld1q_s8_x4(pQLUT + 64);
                int8x16x4_t ql1 = vld1q_s8_x4(pQLUT + 128);
                int8x16x4_t qh1 = vld1q_s8_x4(pQLUT + 192);
...
#define PROCESS_32_ROWS(a_ptr, v_h, v_l, accl_0, acch_0, accl_1, acch_1) { \
                ...
                for (int sub_k = 0; sub_k < 4; sub_k++) {
                    const uint8_t* pA = A + (k + sub_k) * stride + i_packed;
                    int8x16_t vh = (sub_k < 2) ? (sub_k == 0 ? ql0.val[0] : ql0.val[2]) : (sub_k == 2 ? qh0.val[0] : qh0.val[2]);
                    int8x16_t vl = (sub_k < 2) ? (sub_k == 0 ? ql0.val[1] : ql0.val[3]) : (sub_k == 2 ? qh0.val[1] : qh0.val[3]);
                    PROCESS_32_ROWS(pA,      vh, vl, acc[0],  acc[1],  acc[2],  acc[3]);
                    PROCESS_32_ROWS(pA + 16, vh, vl, acc[4],  acc[5],  acc[6],  acc[7]);
                    PROCESS_32_ROWS(pA + 32, vh, vl, acc[8],  acc[9],  acc[10], acc[11]);
                    PROCESS_32_ROWS(pA + 48, vh, vl, acc[12], acc[13], acc[14], acc[15]);
                }
                for (int sub_k = 4; sub_k < 8; sub_k++) {
                    const uint8_t* pA = A + (k + sub_k) * stride + i_packed;
                    int8x16_t vh = (sub_k < 6) ? (sub_k == 4 ? ql1.val[0] : ql1.val[2]) : (sub_k == 6 ? qh1.val[0] : qh1.val[2]);
                    int8x16_t vl = (sub_k < 6) ? (sub_k == 4 ? ql1.val[1] : ql1.val[3]) : (sub_k == 6 ? qh1.val[1] : qh1.val[3]);
                    PROCESS_32_ROWS(pA,      vh, vl, acc[0],  acc[1],  acc[2],  acc[3]);
                    PROCESS_32_ROWS(pA + 16, vh, vl, acc[4],  acc[5],  acc[6],  acc[7]);
                    PROCESS_32_ROWS(pA + 32, vh, vl, acc[8],  acc[9],  acc[10], acc[11]);
                    PROCESS_32_ROWS(pA + 48, vh, vl, acc[12], acc[13], acc[14], acc[15]);
...
#define PROCESS_128_ROWS_INTERLEAVED(k_idx, v_h, v_l) { \
                    const uint8_t* pAk = A + (k + k_idx) * stride + i_packed; \
                    uint8x16_t v0 = vld1q_u8(pAk);      \
                    uint8x16_t v1 = vld1q_u8(pAk + 16); \
                    uint8x16_t v2 = vld1q_u8(pAk + 32); \
                    uint8x16_t v3 = vld1q_u8(pAk + 48); \
                    \
                    uint8x16_t t0 = vshrq_n_u8(v0, 4);  uint8x16_t b0 = vandq_u8(v0, vec_mask); \
                    uint8x16_t t1 = vshrq_n_u8(v1, 4);  uint8x16_t b1 = vandq_u8(v1, vec_mask); \
                    uint8x16_t t2 = vshrq_n_u8(v2, 4);  uint8x16_t b2 = vandq_u8(v2, vec_mask); \
                    uint8x16_t t3 = vshrq_n_u8(v3, 4);  uint8x16_t b3 = vandq_u8(v3, vec_mask); \
                    \
                    uint8x16x2_t u0 = vzipq_u8(t0, b0); \
                    uint8x16x2_t u1 = vzipq_u8(t1, b1); \
                    uint8x16x2_t u2 = vzipq_u8(t2, b2); \
                    uint8x16x2_t u3 = vzipq_u8(t3, b3); \
                    \
                    int8x16_t r0h0 = vqtbl1q_s8(v_h, u0.val[0]); int8x16_t r0l0 = vqtbl1q_s8(v_l, u0.val[0]); \
                    int8x16_t r0h1 = vqtbl1q_s8(v_h, u0.val[1]); int8x16_t r0l1 = vqtbl1q_s8(v_l, u0.val[1]); \
                    int8x16_t r1h0 = vqtbl1q_s8(v_h, u1.val[0]); int8x16_t r1l0 = vqtbl1q_s8(v_l, u1.val[0]); \
                    int8x16_t r1h1 = vqtbl1q_s8(v_h, u1.val[1]); int8x16_t r1l1 = vqtbl1q_s8(v_l, u1.val[1]); \
                    int16x8_t o0, o1, o2, o3; \
                    reconstruct_int16_pair(r0h0, r0l0, o0, o1); \
                    reconstruct_int16_pair(r0h1, r0l1, o2, o3); \
                    acc[0] = vaddq_s16(acc[0], o0); acc[1] = vaddq_s16(acc[1], o1); \
                    acc[2] = vaddq_s16(acc[2], o2); acc[3] = vaddq_s16(acc[3], o3); \
                    reconstruct_int16_pair(r1h0, r1l0, o0, o1); \
                    reconstruct_int16_pair(r1h1, r1l1, o2, o3); \
                    acc[4] = vaddq_s16(acc[4], o0); acc[5] = vaddq_s16(acc[5], o1); \
                    acc[6] = vaddq_s16(acc[6], o2); acc[7] = vaddq_s16(acc[7], o3); \
                    \
                    int8x16_t r2h0 = vqtbl1q_s8(v_h, u2.val[0]); int8x16_t r2l0 = vqtbl1q_s8(v_l, u2.val[0]); \
                    int8x16_t r2h1 = vqtbl1q_s8(v_h, u2.val[1]); int8x16_t r2l1 = vqtbl1q_s8(v_l, u2.val[1]); \
                    int8x16_t r3h0 = vqtbl1q_s8(v_h, u3.val[0]); int8x16_t r3l0 = vqtbl1q_s8(v_l, u3.val[0]); \
                    int8x16_t r3h1 = vqtbl1q_s8(v_h, u3.val[1]); int8x16_t r3l1 = vqtbl1q_s8(v_l, u3.val[1]); \
                    reconstruct_int16_pair(r2h0, r2l0, o0, o1); \
                    reconstruct_int16_pair(r2h1, r2l1, o2, o3); \
                    acc[8] = vaddq_s16(acc[8], o0); acc[9] = vaddq_s16(acc[9], o1); \
                    acc[10] = vaddq_s16(acc[10], o2); acc[11] = vaddq_s16(acc[11], o3); \
                    reconstruct_int16_pair(r3h0, r3l0, o0, o1); \
                    reconstruct_int16_pair(r3h1, r3l1, o2, o3); \
                    acc[12] = vaddq_s16(acc[12], o0); acc[13] = vaddq_s16(acc[13], o1); \
                    acc[14] = vaddq_s16(acc[14], o2); acc[15] = vaddq_s16(acc[15], o3); \
                }

                PROCESS_128_ROWS_INTERLEAVED(0, q0.val[0], q0.val[1]);
                PROCESS_128_ROWS_INTERLEAVED(1, q0.val[2], q0.val[3]);
                PROCESS_128_ROWS_INTERLEAVED(2, q1.val[0], q1.val[1]);
                PROCESS_128_ROWS_INTERLEAVED(3, q1.val[2], q1.val[3]);
#undef PROCESS_128_ROWS_INTERLEAVED
            }
...
int main() {    
    ...
    double avg_vec_packed3_time = benchmark_matmul(
        "\nStep 4: Running LUT Vec Packed3(10 iterations for average)\n",
        "Vecmul_lut_packed3",
        [&]() { vecmul_lut_packed3(A_packed_T, B_T, C_simd, weight_scale, M, N, K); },
        C_simd, M, N, num_iterations
    );
    printf("\nComparing kernel output (C) with reference (C_)...\n");
    ...
    double avg_vec_packed4_time = benchmark_matmul(
        ...
        "Vecmul_lut_packed4",
        [&]() { vecmul_lut_packed4(A_packed_T, B_T, C_simd, weight_scale, M, N, K); },
        C_simd, M, N, num_iterations
    );
    printf("\nComparing kernel output (C) with reference (C_)...\n");
    compare_matrices(C_simd, C_, M, N, 1e-1, "Vecmul_lut_packed4 comparison");

    double avg_vec_packed5_time = benchmark_matmul(
        "\nStep 4: Running LUT Vec Packed5(10 iterations for average)\n",
        "Vecmul_lut_packed5",
        [&]() { vecmul_lut_packed5(A_packed_T, B_T, C_simd, weight_scale, M, N, K); },
        C_simd, M, N, num_iterations
    );
    printf("\nComparing kernel output (C) with reference (C_)...\n");
    ...
    double speedup_vec_packed3 = (double)naive_duration.count() / (double)avg_vec_packed3_time;
    ...
    printf("LUT vecmul_lut_packed3 (avg):   %.2f ms\n", avg_vec_packed3_time);
    printf("Speedup (naive / vecmul_lut_packed3): %.2fx\n\n", speedup_vec_packed3);
    printf("LUT vecmul_lut_packed4 (avg):   %.2f ms\n", avg_vec_packed4_time);
